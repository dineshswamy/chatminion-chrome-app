class @Messages
	init : ->
		...
		@message_options_url=chrome.extension.getBackgroundPage()+"/message_options.json"
		@db_name="calltheteam"
	...
	fetch :()->
    	$.get(@url,(data) => @.addMessage {"id":messages.msg_id,"user_message":messages.user_message,"transform_pattern":messages.transform_pattern} for messages in data)
    	#$.get(@url,(data) => @.addMessageOptions {"id":message_option.id,"message_id":message_option.message_id,"options_id":message_option.options_id} for message_option in data)
